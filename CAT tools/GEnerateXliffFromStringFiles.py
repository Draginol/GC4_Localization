...
def select_files(title):
    root = tk.Tk()
    root.withdraw()
    return filedialog.askopenfilenames(title=title, filetypes=[("XML files", "*.xml")])  # Allows selection of multiple XML files
...
def parse_xml(filepaths, data_dict, translated=False):
...
def generate_xliff(data, language_code, output_directory):
    ...
    for label, ((source, filename, source_priority), target_data) in data.items():
        if not target_data:  # We only want labels that have translations (i.e., are in both source and translated files)
            continue
...
if __name__ == "__main__":
    ...
    translated_files = select_files("Select the translated XML strings files")
    output_directory = select_directory("Select the output directory for the XLIFF file")
    language_code = select_language_code()

    data_dict = {}
    parse_xml([os.path.join(source_directory, file) for file in os.listdir(source_directory) if file.endswith(".xml")], data_dict)
    parse_xml(translated_files, data_dict, translated=True)
